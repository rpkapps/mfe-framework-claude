...
| `@company/mfe-host`           | Neutral orchestration: registry normalization and adapter selection, shell state, validated storage, commands, breadcrumbs, the navigation bridge, auth. `MountController`, the neutral mount controller no shipped adapter uses, is here too. No React, router, single-spa or federation dependency. |
