...
Cut the shipped payload of `@company/mfe-core`, `@company/mfe-host` and `@company/mfe-react`: ~10% off a shell's bundle, ~14% off what every container replicates.
...
- **Breaking:** test doubles (`createInProcessLoader`, `createMemoryNavigationBridge`, `createMemoryStorageArea`, `createRecordingTelemetryProvider`) moved to `@company/mfe-host/testing`; import them from that subpath instead of the package root.
