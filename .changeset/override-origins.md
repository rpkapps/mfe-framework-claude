...
An override for an id the registry does not list is now reported too, rather than changing nothing without a word. And when a different user signs in to a tab than the one its session record was written for, the overrides left in it are not applied and are removed, with a warning; `overrideStorage` accepts an optional `removeItem` for that, and the shell passes the whole of `localStorage`. `establishSessionGeneration` now returns `{ generation, previousIdentity }` so a host that assembles its own runtime can do the same with `discardDevOverrides`.
