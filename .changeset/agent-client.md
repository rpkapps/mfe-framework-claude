...
- `ChatClient` and `fetchServerSentEvents`, with messages as `parts` and approvals as `interrupts`; `subscribe` and `getSnapshot` suit `useSyncExternalStore`. A message keeps its `UIMessage` object until something it shows changes.
- `actionTools`, `approvalsIn` and `agentContextOf` (`/actions`) connect the action registry and agent context.
- `zod` is a peer dependency, as it is of `@ag-ui/core` and of `@company/mfe-runtime`: the host provides the one zod.
...
- `editMessage(id, text, options)` replaces a user message and runs from there, dropping everything after it; it stops a turn in flight and goes through the queue like `reload`. Interrupts of the dropped runs are resumed as cancelled, and an answer owed to them is dropped (now also on `reload`).
