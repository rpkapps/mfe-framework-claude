...
- `PAGE_POLICY` and `withPagePolicy(policy)` are exported. The build adds the page-wide candidates to every profile's `sharing` itself, so a profile lists only its framework's candidates. `adapterDependencies(…).installedVersion` answers only for a name the adapter declares.
