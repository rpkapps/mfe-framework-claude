...
`default` is always registered, first, because the page singletons live there. An entry without `shareScopes`, from a container built before framework scopes, is registered with `default` alone, as before.
