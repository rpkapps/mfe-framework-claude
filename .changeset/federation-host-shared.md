...
`@company/mfe-rspack/federation` exports `hostShared({ root })`, the `shared` map for a federation host's `moduleFederation.options`, built from the same candidate list, singleton/eager rules and version rule `pluginMfe()` gives a container.
