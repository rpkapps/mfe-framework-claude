...
export function stylesheetFile(context: GenerateContext): GeneratedFile {
  ...
  return {
    ...
    contents: joinBlocks([
      ...
      [
        '/*',
        ' * What Tailwind scans. `source(none)` above turns automatic source',
        ' * detection off, which would otherwise scan the directory the build runs',
        ' * in — the container, or a whole workspace — and hand every directory it',
        ' * walked to the bundler to watch: an Angular dev server then rebuilt the',
        ' * container after every rebuild. So every directory holding classes this',
        ' * container renders has to be named here — the library scans its own.',
        " * The one entry below covers the whole of this container's `src/`;",
        ' * nothing under node_modules needs scanning.',
        ' */',
