...
import { ADAPTER_MODULES } from './definitions.ts'
...
export function resolveAppRoutes(entryFile: string): AppRoutes {
  ...
  for (const [local, binding] of imports) {
    if (binding.moduleSpecifier === ADAPTER_MODULES.angular && binding.imported === 'createApp') {
      factories.add(local)
    }
  }
...
export function routeDataCalls(sourceFile: ts.SourceFile): readonly ts.CallExpression[] {
  ...
  for (const [local, binding] of collectImportedBindings(sourceFile)) {
    if (binding.moduleSpecifier !== ADAPTER_MODULES.angular) continue
...
export function routeDataPath(
  ...
  const reject = (anchor: ts.Node, expected: string, observed: string, repair: string): never => {
  ...
  if (
    ...
    return reject(
      ...
      `Declare the route inline with the call as its data, for example { path: 'settings', loadComponent: () => import('./settings.component.ts'), data: ${ROUTE_DATA_FACTORY}({ capability: 'settings', label: 'Order settings' }) }.`,
    )
  }
  ...
  for (;;) {
    ...
    if (!ts.isArrayLiteralExpression(array)) {
      return reject(
        ...
        `a route object in ${describeNode(sourceFile, outerExpression(current).parent)}`,
    ...
    if (!isAppRoutes(sourceFile, array, appRoutes)) {
      return reject(
        ...
        'Move the capability route into the routes array createApp receives, or into an inline children: [ … ] of a route there, and keep it lazy with loadComponent. An array reached through an identifier, a spread or loadChildren cannot be traced to a URL at build time.',
      )
    }
...
function literalPath(
  ...
  reject: (anchor: ts.Node, expected: string, observed: string, repair: string) => never,
): string {
