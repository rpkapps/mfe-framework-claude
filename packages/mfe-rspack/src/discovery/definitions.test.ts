...
describe('discoverDefinitions', () => {
  it('reads an App with its id and version', () => {
    ...
    const result = discoverDefinitions(entryOf(root))
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/named exports/)
  })
...
`,
    ...
    try {
      discoverDefinitions(entryOf(root))
...
export const one = createApp({ id: 'one', router: makeRouter })
export const two = createApp({ id: 'two', router: makeRouter })
`,
    })

    expect(() => discoverDefinitions(entryOf(root))).toThrow(/at most one App per container/)
  })
...
const suffix = 'x'
export const app = createApp({ id: \`operations-\${suffix}\`, router: makeRouter })
`,
    })

    expect(() => discoverDefinitions(entryOf(root))).toThrow(/plain string literal/)
  })
...
export const app = createApp({ id: 'Operations Team', router: makeRouter })
`,
    })

    expect(() => discoverDefinitions(entryOf(root))).toThrow(/storage prefix/)
  })
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/reserved/)
  })
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/event handler/)
  })
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/lower-camel-case/)
  })
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/onSelected/)
  })
...
`,
    ...
    expect(() => discoverDefinitions(entryOf(root))).toThrow(/exported/)
  })

  it('recognises a factory imported under an alias', () => {
    const root = createContainer({
      'src/mfe.ts': `
import { createWidget as make } from '@company/mfe-react'
import { z } from 'zod'
...
`,
    ...
    expect(discoverDefinitions(entryOf(root)).widgets[0]?.id).toBe('row')
  })

  it('reports a container entry that exports no definition', () => {
    const root = createContainer({ 'src/mfe.ts': 'export const nothing = 1\n' })

    expect(() => discoverDefinitions(entryOf(root))).toThrow(/at least one exported App or Widget/)
  })
})
...
`
  ...
  it('reports it as a build error instead of ignoring it silently', () => {
    const root = createContainer({ 'src/mfe.ts': APP, 'src/widgets/hidden.ts': STRAY })
    const errors = findStrayDefinitions(`${root}/src`, { entryFile: entryOf(root) })
  ...
  it('leaves test files alone', () => {
    ...
    expect(findStrayDefinitions(`${root}/src`, { entryFile: entryOf(root) })).toHaveLength(0)
  })
})
