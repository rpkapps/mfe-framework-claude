/**
 * The scoping recipe for a container whose build cannot resolve the design system's own plugin,
 * such as an Angular container: every rule is confined to the container's mount roots with CSS
 * `@scope`, and what `@scope` cannot contain is made unique instead.
 */
...
/** The same options the design system's plugin takes, so either can scope a container. */
