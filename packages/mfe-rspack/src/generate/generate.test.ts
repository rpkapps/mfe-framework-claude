...
/*
 * What a React container's generated output adds to the neutral build's, whose own suite covers
 * the rest: the design system, the React adapter and the file routes.
 */
...
describe('the generated banners', () => {
  it('name this plugin as the place to change what it wrote', () => {
    const { fileFor } = planFixture({ 'src/mfe.ts': APP_ENTRY }, TECTON_MANIFEST)

    for (const file of ['styles.css', 'fetch.ts', 'entries/app.ts', 'entries/style-root.tsx']) {
      expect(fileFor(file), file).toContain(
        ' * Generated by @company/mfe-rspack. Do not edit: the next build overwrites it.',
      )
    }
    expect(fileFor('.gitignore')).toBe(
      '# Generated by @company/mfe-rspack. Everything here is build output, except the\n' +
