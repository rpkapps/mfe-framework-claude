...
describe('aroundLoad', () => {
  it('wraps each remote evaluation, and nothing else', async () => {
    const events: string[] = []
    const { runtime, registerRemotes, loadRemote } = createRuntime({
      example_reports: () => {
    ...
    expect(events).toEqual(['register', 'before', 'evaluate', 'after'])
    expect(loadRemote).toHaveBeenCalledTimes(1)
  })

  it('maps a failure raised inside it like any other load failure', async () => {
    const { runtime } = createRuntime({
      example_reports: () => Promise.reject(manifestError('http://localhost:9999/m.json')),
    })
    const loader = createFederationContainerLoader({ runtime, aroundLoad: load => load() })
